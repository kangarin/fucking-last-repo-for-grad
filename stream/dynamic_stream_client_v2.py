...
class DynamicStreamClient:
    def __init__(self, stream_id=None):
        ...
        self.current_fps_range = self.generate_random_fps_range()
    ...
    def generate_random_fps_range(self):
        """生成随机的FPS范围，确保最小值和最大值之间有足够的差异"""
        # 计算配置的FPS范围
        config_fps_range = self.fps_config['max_fps'] - self.fps_config['min_fps']
        # 要求最小和最大FPS之间的差值至少是总范围的30%
        min_difference = config_fps_range * 0.3
        
        while True:
            # 随机生成两个FPS值
            fps1 = random.uniform(self.fps_config['min_fps'], self.fps_config['max_fps'])
            fps2 = random.uniform(self.fps_config['min_fps'], self.fps_config['max_fps'])
            
            # 计算差值
            difference = abs(fps1 - fps2)
    ...
    def calculate_target_fps(self, elapsed_time, cycle_duration):
        ...
        fps_range = self.current_fps_range['max_fps'] - self.current_fps_range['min_fps']
        target_fps = (sine_val + 1) / 2 * fps_range + self.current_fps_range['min_fps']
